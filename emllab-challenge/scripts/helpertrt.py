...
def _do_inference_base(inputs, outputs, stream, execute_async):
    ...
    #print(f"inp to gpu: \t {(time.time() - test)*1000:.2f} ms") 
    
    #test = time.time()
    # Run inference.
    execute_async()
    #print(f"infer: \t {(time.time() - test)*1000:.2f} ms") 
    
    #test = time.time()
    # Transfer predictions back from the GPU.
    kind = cudart.cudaMemcpyKind.cudaMemcpyDeviceToHost
    [cuda_call(cudart.cudaMemcpyAsync(out.host, out.device, out.nbytes, kind, stream)) for out in outputs]
    #print(f"pre to gpu: \t {(time.time() - test)*1000:.2f} ms") 
    
    #test = time.time()
    # Synchronize the stream
    cuda_call(cudart.cudaStreamSynchronize(stream))
    #print(f"sync stream: \t {(time.time() - test)*1000:.2f} ms") 
