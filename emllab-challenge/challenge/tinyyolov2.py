...
class TinyYoloV2_BNOpt(nn.Module):
    ...
    def _sd_hook(self, state_dict, prefix, *_):
        """Reinitialize all layers with the dimensions of the state_dict being
        loaded.
        """
        for n in range(1, 9):
            dim_conv_in = state_dict[f'conv{n}.weight'].shape[1]
            dim_conv_out = state_dict[f'conv{n}.weight'].shape[0]
            conv = nn.Conv2d(dim_conv_in, dim_conv_out, 3, 1, 1)
            setattr(self, f'conv{n}', conv)

        dim_in = state_dict[f'conv9.weight'].shape[1]
        self.conv9 = nn.Conv2d(dim_in, len(self.anchors) * (5 + self.num_classes), 1, 1, 0)




class TinyYoloV2_pruned(TinyYoloV2):
    """Version of the TinyYoloV2 network that can be used with pruned state 
    dicts.

    Before loading the state_dict, all layers are reinitialized with the
    ...
    def _sd_hook(self, state_dict, prefix, *_):
        ...
        for n in range(1, 9):
            dim_conv_in = state_dict[f'conv{n}.weight'].shape[1]
            dim_conv_out = state_dict[f'conv{n}.weight'].shape[0]
            dim_bn = state_dict[f'bn{n}.weight'].shape[0]

            conv = nn.Conv2d(dim_conv_in, dim_conv_out, 3, 1, 1, bias=False)
            bn = nn.BatchNorm2d(dim_bn)
